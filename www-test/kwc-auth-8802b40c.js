  ...
  */
  ...
  class DomApiNative {
    ...
    unobserveNodes(observerHandle) {
      observerHandle && observerHandle.disconnect();
    }
...
`;
  ...
  */
  ...
  const html$1 = function html(strings, ...values) {
      ...
      template.innerHTML = values.reduce((acc, v, idx) =>
          acc + htmlValue$1(v) + strings[idx + 1], strings[0]);
      return template;
  };
...
`;
  ...
  document.head.appendChild(colors.content);

  const buttonColors = html$1`
    <style>
        html {
...
`;
  ...
  */
  ...
  class KwcAuth extends mixinBehaviors([Behaviour], PolymerElement) {
      static get template() {
          return html`
        ${button}
        <style>
