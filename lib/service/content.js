...
app.factory('contentService', function ($http, $q, $rootScope, _config) {
    function getWorlds () {
        var defer = $q.defer(),
        url =  i18n.getChallengeLocalePath(_config.APP_ROOT, config.CHALLENGES_URL) + "/index.json";
        $http({
            method: 'GET',
            url: url
    ...
    function getWorld(contentUrl) {
        var defer = $q.defer(),
        url =  i18n.getChallengeLocalePath(_config.APP_ROOT, config.CHALLENGES_URL) + "/" + contentUrl + "/index.json";
        $http({
            method: 'GET',
            url: url
        }).then(function successCallback(response) {
    ...
    function getChallenge(world, challenge_id) {
        var defer = $q.defer(),
        url =  i18n.getChallengeLocalePath(_config.APP_ROOT, config.CHALLENGES_URL) + "/worlds/" + world + "/" + challenge_id + ".json";
        $http({
            method: 'GET',
            url: url
        }).then(function successCallback(response) {
