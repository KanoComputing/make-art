...
app.factory('contentService', function ($http, $q, $rootScope) {
    function getWorlds () {
        var defer = $q.defer(),
        url = config.CHALLENGES_URL + i18n.getLocalePath() + "/index.json";
        $http({
            method: 'GET',
            url: url
    ...
    function getWorld(contentUrl) {
        var defer = $q.defer(),
        url = config.CHALLENGES_URL + i18n.getLocalePath() + "/" + contentUrl + "/index.json";
        $http({
            method: 'GET',
            url: url
        }).then(function successCallback(response) {
    ...
    function getChallenge(world, challenge_id) {
        var defer = $q.defer(),
        url = config.CHALLENGES_URL + i18n.getLocalePath() + "/worlds/" + world + "/" + challenge_id + ".json";
        $http({
            method: 'GET',
            url: url
        }).then(function successCallback(response) {
