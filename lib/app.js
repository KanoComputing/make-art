...
app.run(function ($rootScope, $window) {
    ...
    if (!localStorage.groups) {
        $rootScope.progress.groups['summercamp'] = {};
        $rootScope.progress.groups['summercamp'].challengeNo = parseInt(0, 10);
    }
    else {
        $rootScope.progress.groups['summercamp'] = localStorage.groups['summercamp'] || {};
        $rootScope.progress.groups['summercamp'].challengeNo = parseInt(localStorage.groups['summercamp']['challengeNo'] || 0, 10);
    }
    ...
    auth.init(function () {
        ...
        // Load progress
        api.progress.load(function (challengeNo, group) {
            $rootScope.progress.challengeNo = challengeNo || 1;
            if (groups) {
                $rootScope.progress.groups['summercamp']['challengeNo'] = groups['summercamp']['challengeNo'];
            }
            else {
                $rootScope.progress.groups['summercamp']['challengeNo'] = 1;
            }
            $rootScope.$apply();
        });
