...
export const app = angular.module('draw', ['ngRoute']);
...
app.run(($rootScope, $window, $location) => {
    ...
    $rootScope.updateProgress = function (worldId, challengeNo) {
        if (!worldId || challengeNo === undefined) {
            console.log("!worldId,challengeNo =>", !worldId, challengeNo );
            $rootScope.xpGain = 0;
            $rootScope.$apply();
            return;
        }
        console.log("$rootScope.progress.groups =>", $rootScope.progress.groups);
