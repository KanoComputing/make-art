...
import palette from '../language/modules/palette.js';
import allDocs from '../content/docs.js';
import equal from '../util/deep-equal.js';
import domUtil from '../util/dom.js';
import stringUtil from '../util/string.js';
import { ace } from '../modules/ace.js';
import numberTokenHanlder from '../editor/token-handler/NumberHandler.js';
import colorTokenHanlder from '../editor/token-handler/ColorHandler.js';

const docs = allDocs[i18n.getLanguage() || 'en'];
...
let autoCompleteActive = localStorage.autocomplete === 'false' ? false : true;

app.directive('editor', ['$rootScope', '$controller', '$compile', '$routeParams', 'contentService', '_config', function ($rootScope, $controller, $compile, $routeParams, contentService, _config) {
    ace.config.set('basePath', `${_config.UI_ROOT}lib/vendor/ace/`);
    var config;


    let tokenHandlers = {
        number: numberTokenHanlder,
        color: colorTokenHanlder,
    },
    ...
    function initialiseEditor() {
        langTools.setCompleters([{
            getCompletions: getAutoComplete,
        }]);
    }
    ...
    return {
        require: '?workspace',
        restrict: 'E',
        templateUrl: i18n.getHtmlLocalePath(_config.APP_ROOT) + '/directive/editor.html',
        scope: {
            ngModel: '=',
            editable: '=',
            autoSelect: '=',
