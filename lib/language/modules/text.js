...
function text(val) {
    ...
    label.style.fontFamily = session.settings.text.font;
    label.style.fontSize = session.settings.text.size + 'px';
    label.style.color = session.settings.fill;
    if (session.settings.text.bold) {
        label.style.fontWeight = 'bold';
    }
    if (session.settings.text.italic) {
    ...
    label.style.top = session.pos.y - (rect.height) + 'px';
    label.style.left = session.pos.x - (rect.width / 2) + 'px';
    id++;
    return {
