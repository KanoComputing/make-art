...
/*
 * Set current session stroke cap
 *
 * @param {String} val
 * @return void
 */
function strokeCap(val) {
    session.settings.stroke.cap = val;
    session.ctx.lineCap = val;
    utils.record('stroke-cap', { width: val });
}
...
function stroke() {
    ...
    if (style.cap) { strokeCap(style.cap); }
