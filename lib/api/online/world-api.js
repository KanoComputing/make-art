...
export default function (config) {
	...
	return {
		...
		share: {
			getBySlug: (slug) => {
				return fetch(`${config.API_URL_V2}/shares?slug=${slug}`, {
					headers: getHeaders(),
				})
					.then(res => res.json())
					.then((res) => {
			...
			post: (data) => {
				...
				.then((res) => {
					return fetch(`${config.API_URL_V2}/${res.path}`, { headers: getHeaders() })
						.then(r => r.json())
						.then((r) => {
							const { share, atBurl } = r.data;
							const creation = share;
							creation.cover_url = `${atBurl}/${share.id}/cover.png`;
							return {
								body: {
									item: share,
								},
