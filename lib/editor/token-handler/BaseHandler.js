...
BaseHandler.prototype.open = function () {
    ...
    var self = this;
    this.ng.$controller(class { constructor() { self.scope.$apply(() => self.init()); } });
    this.resize();
};
...
BaseHandler.prototype.getTokenElement = function () {
    ...
    if (lineElement.childNodes[this.token.index].nodeName === '#text') {
        this.token.index = [...lineElement.childNodes].findIndex(lineElement.childNodes, function (el, index) {
