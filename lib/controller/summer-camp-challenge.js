...
app.controller('SummerCampChallengeController', function ($scope, $routeParams, $window, $timeout, $rootScope, $location) {
    ...
    var progressNo = $rootScope.progress.groups['summercamp'] ? $rootScope.progress.groups['summercamp'].challengeNo : 1;
    if (($scope.id - progressNo) > 1) {
        $location.path('/summercamp/calendar');
    }
    else {
    ...
    $rootScope.$watch('successful', function (msg) {
        if (msg) {
            $rootScope.updateProgress($scope.id + 1, 'summercamp');
