...
app.controller('ChallengesController', function ($scope, $rootScope, $routeParams, $location, contentService) {
    ...
    $scope.loadWorld = function (world) {
        var i, worldPath = "worlds/" + world.id;
        if (!$scope.isWorldLocked(world)) {
            contentService.getWorld(worldPath).then(function (data) {
                ...
                if ($rootScope.selectedWorld.id === "mischiefweek2015") {
                    for (i in $scope.challenges) {
                        $scope.challenges[i].day = $scope.weekdays[i];
                    }
                }
            });
        }
    ...
    */
    ...
    $scope.isCompleted = function (challenge) {
        return challenge.index < inWorldProgress.challengeNo;
    };
