...
app.controller('ChallengesController', function ($scope, $rootScope, $routeParams, $location, contentService, securityService) {
    ...
    $scope.loadWorld = function (world) {
        ...
        if (!$scope.isWorldLocked(world)) {
            contentService.getWorld(worldPath).then(function (data) {
                ...
                $rootScope.inWorldProgress = securityService.progress.get($rootScope.selectedWorld.id);
    ...
    $scope.isWorldCompleted = function (world) {
        return securityService.world.isCompleted(world);
    };
    ...
    */
    $scope.isWorldLocked = function (world) {
        return securityService.world.isLocked(world);
    };
    ...
    $scope.isWorldCurrent = function (world) {
        return securityService.world.isCurrent(world);
    };
    ...
    $scope.isLocked = function (challenge) {
        return securityService.challenge.isLocked(challenge);
    };
     ...
     * @return {Boolean}
     */
    $scope.isCompleted = function (challenge) {
        return securityService.challenge.isCompleted(challenge);
    };
     ...
     * @return {Boolean}
     */
    $scope.isCurrent = function (challenge) {
        return securityService.challenge.isCurrent(challenge);
    };
