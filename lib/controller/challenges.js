...
app.controller('ChallengesController', function($scope, $rootScope, $routeParams, $location, $http, contentService, _config, HandleActivationService) {
    ...
    function init() {
        ...
        contentService.world.getAll().then(function gotWorlds(data) {
            ...
            if (HandleActivationService.AreArguementsQueuedForProcessing()) {
                console.log('We have some arguments for the handleActivation!q')
                HandleActivationService.TestAndHandleArgs()
            } else {
                // 
            }

            if (routeWorld && !$scope.isWorldLocked(routeWorld)) {
                console.log('!$scope.isWorldLocked(routeWorld)=>',!$scope.isWorldLocked(routeWorld))
