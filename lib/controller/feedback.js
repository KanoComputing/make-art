...
app.controller('FeedbackController', function ($scope, $rootScope, $routeParams) {
    ...
    .then(function (res) {
        if (res.body.questions) {
            $scope.questions = _.filter(res.body.questions, { tags: ['feedback'] });
            $scope.$apply();
        }
    });
    ...
    $scope.sendFeedback = function () {
        ...
        api.questions.respond({
            username: $rootScope.user ? $rootScope.user.username : $rootScope.cfg.FEEDBACK_ACCOUNT,
            email: $rootScope.user ? $rootScope.user.email : $rootScope.cfg.FEEDBACK_ACCOUNT + '@kano.me',
            answers: answers
        })
        .then(function () {
            handleClick();
