...
app.controller('SummerCampController', function ($interval, $timeout, $scope, $rootScope, $location, emailService) {
    ...
    // Call the mail share function
    $scope.sendMail = function (challengeObj) {
        var emailObj;
        if (!challengeObj.name) { return false; }
        if (!challengeObj.message) { return false; }
        if (!challengeObj.email) {
            return false;
        } else {
            if (validateEmail(challengeObj.email)) {
                challengeObj.user_email = $rootScope.user.email;
                emailObj = buildEmailObject(challengeObj);

                emailer(emailObj, function (response) {
                    if (response.status === 200) {
                        resetFields(challengeObj);
                        $scope.mailPreview = false;
    ...
    //     var req = {
    //         method  : 'POST',
    //         url     : 'http://localhost:7000/summercamp/send',
    //         headers : {
